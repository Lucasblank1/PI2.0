...
<head>
    ...
    <title><?php echo htmlspecialchars($historia['titulo']); ?> - Caça aos fatos</title>
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-3xl mx-auto">
                ...
                <div class="bg-amber-100 rounded-xl p-8 shadow-lg story-card mb-8">
                    <?php
                    // Exibir imagem para cada capítulo da história 'O Período Colonial'
                    if ($historia_id === 'periodo_colonial') {
                        $imagens_periodo_colonial = [
                            'chegada.jpeg',
                            'exploracao.png',
                            'primeiro-contato.jpeg',
                            'desfecho.jpeg',
                            'desfecho-pacifico.jpeg'
                        ];
                        if (isset($imagens_periodo_colonial[$capitulo_atual])) {
                            $img = $imagens_periodo_colonial[$capitulo_atual];
                            echo '<div class="flex justify-center mb-6">';
                            echo '<img src="assets/images/' . $img . '" alt="' . htmlspecialchars($capitulo['titulo']) . '" style="width:100%; max-width:400px; height:280px; object-fit:contain; border-radius:20px; box-shadow:0 4px 24px rgba(0,0,0,0.10);">';
                            echo '</div>';
                        }
                    } else {
                        // Exibe o conteúdo normalmente para outras histórias
                        echo '<h2 class="text-2xl font-bold text-amber-800 mb-4">' . htmlspecialchars($capitulo['titulo']) . '</h2>';
                        echo '<p class="text-amber-700 text-lg mb-6">' . htmlspecialchars($capitulo['conteudo']) . '</p>';
                    }
                    ?>
                    <div class="space-y-4">
